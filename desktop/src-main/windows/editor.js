...
class EditorWindow extends ProjectRunningWindow {
  ...
  constructor (initialFile, isInitiallyFullscreen) {
    ...
    // クソ重い(大量クローン等)プロジェクトでもマシに動くように、
    // 軽量寄りの設定を「デフォルトで強制」する。設定画面は一切触らせない。
    // fps=30: 60fpsの半分の負荷 / interpolateは付けない(補間は負荷増)
    // hqpenは付けない(高画質ペンOFF=軽い)
    // turboは挙動(wait/タイミング)が変わってしまうため付けない。
    this.loadURL('tw-editor://./gui/gui.html?fps=30');
    this.show();
  }
